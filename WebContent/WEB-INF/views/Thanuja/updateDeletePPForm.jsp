...
<html lang="zxx" class="no-js">
		...
		<%
			PoliticalParty p = (PoliticalParty)request.getAttribute("poliP");
		%>
			  
<div class="containerForm">
  <form id="contact" method="POST" action="ThanujaUpdatePPServlet">
    <h3>Political party</h3>
    
    
    <fieldset>
    	<label> Party logo : </label>
		<input type = "text" name = "partyLogo" value ="<%=p.getPartyLogo()%>">
    </fieldset>
    
    <fieldset>
     	<label> Party name : </label>
		<input type = "text" name = "partyName" value = "<%=p.getPartyName()%>">
    </fieldset>
    
    <fieldset>
     	<label> Abbreivation : </label>
		<input type = "text" name = "abbreivation" value = "<%=p.getAbbreivation()%>">
    </fieldset>
    
    <fieldset>
     	<label> Leader name : </label>
		<input type = "text" name = "ledName" value = "<%=p.getLeaderName()%>">
    </fieldset>
    
	<fieldset>
     	<label> Secretary name : </label>
		<input type = "text" name = "secName" value = "<%=p.getSecName()%>">
    </fieldset>
    
	<fieldset>
    	<label> Official contact : </label>
		<input type = "text" name = "offContact" value = "<%=p.getOfficialContact()%>">
    </fieldset>
    
	<fieldset>
     	<label> Official email : </label>
		<input type = "text" name = "offEmail" value = "<%=p.getOfficialEmail()%>">
    </fieldset>
    
	<fieldset>
     	<label> Address number : </label>
		<input type = "text" name = "addNo" value = "<%=p.getAddressNo()%>">
    </fieldset>
    
	<fieldset>
     	<label> Address street : </label>
		<input type = "text" name = "addStreet" value = "<%=p.getAddressStreet()%>">
    </fieldset>
    
	<fieldset>
      <!-- <input placeholder="<%=p.getAddressCity() %>" type="text" tabindex="10" name = "addCity">-->
     	<label> Address city : </label>
		<input type = "text" name = "addCity" value = "<%=p.getAddressCity()%>">
    </fieldset>

    <fieldset>
    	<input type = "hidden" name = "politiId" value = "<%=p.getPartyId()%>"/>
     	<button name="re" type="submit" id="contact-submit" data-submit="...Sending">Update</button> 
    </fieldset>    
  </form>
  
  <form id="contact" method="POST" action="ThanujaDeletePPServlet">
  	<fieldset>
    	<input type = "hidden" name = "poliId" value = "<%=p.getPartyId()%>"/>
     	<button name="re" type="submit" id="contact-submit" data-submit="...Sending">Delete</button> 
   	</fieldset>
  </form>
</div>
