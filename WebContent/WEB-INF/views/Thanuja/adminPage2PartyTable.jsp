...
<div class="container">
           ...
           <div class="table-title">
                <div class="row">
                    <div class="col-sm-6">
						<h2>Manage <b>political parties</b></h2>
					</div>
					<div class="col-sm-6">
						<a href="thAddPPForm" class="btn btn-success"><span>Add new party</span></a>
					</div>
                </div>
            </div>
        ...
        <table class="table" id="table1">
          ...
          <tbody>
			...
			<%
				...
				ArrayList<PoliticalParty> al = th.getAllPoliticalParties();
				
				for(PoliticalParty p : al){
			%>
		  
            <tr>
              <td><%=p.getPartyId() %></td>
              <td><%=p.getPartyLogo() %></td>
              <td><%=p.getPartyName() %></td>
              <td><%=p.getAbbreivation() %></td>
              <td><%=p.getLeaderName() %></td>
              <td><%=p.getSecName()%></td>
              <td><%=p.getOfficialContact() %></td>
              <td><%=p.getOfficialEmail() %></td>
              <td><%=p.getAddressNo() %></td>
              <td><%=p.getAddressStreet() %></td>
              <td><%=p.getAddressCity() %></td>
			  <td>
				<form method="POST" action="ThanujaGetPPServlet">
					<input type = "hidden" name = "pId" value = "<%=p.getPartyId()%>"/>
